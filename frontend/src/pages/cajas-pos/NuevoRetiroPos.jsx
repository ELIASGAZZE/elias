...
const NuevoRetiroPos = () => {
  ...
  const confirmarRetiro = async () => {
    ...
    try {
      ...
      const { data } = await api.post(`/api/cierres-pos/${id}/retiros`, {
  ...
  if (cargando) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Navbar titulo="Nuevo Retiro POS" sinTabs volverA={`/cajas-pos/cierre/${id}`} />
  ...
  if (error && !cierre) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Navbar titulo="Nuevo Retiro POS" sinTabs volverA="/cajas-pos" />
    ...
    <div className="min-h-screen bg-gray-50 pb-6">
      ...
      <div className="px-4 py-4 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between bg-teal-50 border border-teal-200 rounded-xl p-3 mb-4">
          <div className="text-sm text-teal-800">
            <p className="font-semibold">
              Sesion POS{cierre.apertura_at ? ` — ${formatHora(cierre.apertura_at)}` : ''}
            </p>
            <p>Caja: {cierre.caja?.nombre || '-'} · Empleado: {cierre.empleado?.nombre || '-'} · {formatFecha(cierre.fecha)}</p>
          </div>
          <div className="text-right">
            <span className="text-xs text-teal-600">Total retiro</span>
            <p className="text-lg font-bold text-teal-700">{formatMonto(totalRetiro)}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
          <label className="text-sm font-medium text-gray-700 mb-2 block">Codigo de empleado que retira</label>
          <input
            type="text"
