...
const PedidosPOS = ({ embebido, onEntregarPedido, onEditarPedido }) => {
  ...
  const cargarPedidos = useCallback(async () => {
    setCargando(true)
    try {
      const { data } = await api.get('/api/pos/pedidos', { params: { estado: filtroEstado } })
  ...
  // Filtro local: busca en cliente, ID, productos, observaciones
  const pedidosFiltrados = useMemo(() => {
    if (!busqueda.trim()) return pedidos
    const terminos = busqueda.toLowerCase().trim().split(/\s+/)
    return pedidos.filter(p => {
      const items = typeof p.items === 'string' ? JSON.parse(p.items) : p.items
      const productosTexto = items.map(i => i.nombre).join(' ')
      const texto = [
        p.nombre_cliente,
        p.id,
        productosTexto,
  ...
  return (
    <div className={embebido ? 'h-full bg-gray-50 flex flex-col overflow-hidden' : 'min-h-screen bg-gray-50'}>
      ...
      {/* Filtros */}
      <div className={embebido ? 'px-4 pt-3 flex items-center justify-between' : 'max-w-4xl mx-auto px-4 pt-4'}>
        ...
        {embebido && (
          <button onClick={cargarPedidos} className="text-sm text-violet-600 hover:text-violet-700 font-medium flex-shrink-0">
            Actualizar
          </button>
        )}
      </div>
      ...
      {/* Buscador */}
      <div className={embebido ? 'px-4 pt-3' : 'max-w-4xl mx-auto px-4 pt-3'}>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar por cliente, producto, observaciones..."
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1 text-xs text-gray-400">
                  ...
                  {pedidoDetalle.esPagado && (
                    <span className="px-1.5 py-0.5 rounded font-medium bg-green-50 text-green-700">Pagado</span>
                  )}
