...
router.get('/', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    logger.error('Error al obtener api_logs:', err)
...
router.get('/health', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    logger.error('Error al obtener health:', err)
...
router.get('/errores-recientes', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    logger.error('Error al obtener errores recientes:', err)
...
router.post('/sync/clientes-bi', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    const { syncClientesRecientes } = require('../services/centumClientes')
    const resultado = await syncClientesRecientes(2)
    res.json({ ok: true, ...resultado })
  } catch (err) {
    logger.error('Error en sync clientes BI manual:', err)
...
router.post('/sync/clientes-retry', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    const { retrySyncCentum } = require('../services/centumClientes')
    const resultado = await retrySyncCentum()
    res.json({ ok: true, ...resultado })
  } catch (err) {
    logger.error('Error en retry clientes manual:', err)
    res.status(500).json({ error: err.message })
  }
}))
...
router.post('/sync/clientes-faltantes', verificarAuth, soloAdmin, asyncHandler(async (req, res) => {
  try {
    const { syncClientesFaltantes } = require('../services/centumClientes')
    const resultado = await syncClientesFaltantes()
    res.json({ ok: true, ...resultado })
  } catch (err) {
    logger.error('Error en sync clientes faltantes manual:', err)
    res.status(500).json({ error: err.message })
  }
}))
