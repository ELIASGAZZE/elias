...
router.get('/combos-erp', verificarAuth, soloGestorOAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    while (true) {
      const { data, error } = await supabase
        .from('articulos')
        .select('id, id_centum, codigo, nombre, precio, rubro, marca, articulos_por_sucursal(habilitado)')
        .eq('tipo', 'combo')
        .order('nombre')
        .range(from, from + PAGE_SIZE - 1)
      if (error) throw error
    ...
    const combos = allData.map(c => ({
      id: c.id,
      id_centum: c.id_centum,
      codigo: c.codigo,
      nombre: c.nombre,
      precio: c.precio,
      rubro: c.rubro,
      marca: c.marca,
      habilitado: (c.articulos_por_sucursal || []).some(s => s.habilitado),
    }))
...
router.post('/combos-toggle', verificarAuth, soloGestorOAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    if (!id || typeof habilitado !== 'boolean') {
      return res.status(400).json({ error: 'Se requiere id y habilitado (boolean)' })
    }

    // Verificar que es un combo
    const { data: art } = await supabase.from('articulos').select('id, tipo, nombre').eq('id', id).single()
    if (!art) return res.status(404).json({ error: 'Artículo no encontrado' })
    if (art.tipo !== 'combo') return res.status(400).json({ error: 'Solo se pueden habilitar/deshabilitar combos' })

    // Actualizar habilitado en todas las sucursales
