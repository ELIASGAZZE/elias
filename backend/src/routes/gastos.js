// Rutas de gastos durante turno de caja
...
// POST /api/cierres/:cierreId/gastos — crear gasto
router.post('/cierres/:cierreId/gastos', verificarAuth, validate(crearGastoSchema), asyncHandler(async (req, res) => {
  try {
...
// GET /api/cierres/:cierreId/gastos — listar gastos de un cierre
router.get('/cierres/:cierreId/gastos', verificarAuth, asyncHandler(async (req, res) => {
  try {
    const { data: cierre, error: errorCierre } = await supabase
      .from('cierres')
...
// PUT /api/gastos/:id/controlar — marcar gasto como controlado (gestor/admin)
router.put('/gastos/:id/controlar', verificarAuth, soloGestorOAdmin, validate(controlarGastoSchema), asyncHandler(async (req, res) => {
  try {
    const { data: gasto, error: errorGasto } = await supabase
      .from('gastos')
      .select('id, cierre_id, controlado, cierre:cierres!cierre_id(id, cajero_id, caja_id, caja:cajas(id, sucursal_id))')
      .eq('id', req.params.id)
      .single()
...
// DELETE /api/gastos/:id — eliminar gasto (solo si caja abierta)
router.delete('/gastos/:id', verificarAuth, asyncHandler(async (req, res) => {
  try {
    const { data: gasto, error: errorGasto } = await supabase
      .from('gastos')
      .select('id, cierre_id, cierre:cierres!cierre_id(id, cajero_id, estado)')
      .eq('id', req.params.id)
      .single()
